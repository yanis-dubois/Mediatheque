...
pub fn get_collection_by_id(
  ...
  let collection = stmt
    .query_row([&collection_id], map_row_to_collection)
    .map_err(|e| e.to_string())?;
...
pub fn get_collection_batch(
  ...
  let list = stmt
    .query_map(rusqlite::params_from_iter(ids), map_row_to_collection)
    .map_err(|e| e.to_string())?
...
#[tauri::command]
pub fn toggle_collection_favorite(
  state: tauri::State<'_, DbState>,
  id: String,
  is_favorite: bool,
) -> Result<(), String> {
...
#[tauri::command]
pub fn update_collection_preferred_layout(
  state: tauri::State<'_, DbState>,
  id: String,
...
pub fn update_collection_sort(
  ...
  // Vec<MediaOrder> -> JSON
  let sort_order_json = serde_json::to_string(&sort).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn update_collection_filter(
  state: tauri::State<'_, DbState>,
  id: String,
  filter: MediaFilter,
) -> Result<(), String> {
...
#[tauri::command]
pub fn update_collection_media_type(
  state: tauri::State<'_, DbState>,
  id: String,
  media_type: CollectionMediaType,
) -> Result<(), String> {
...
#[tauri::command]
pub fn add_media_batch_to_collection(
  state: tauri::State<'_, DbState>,
  id: String,
...

  {
    ...
    let mut pos_stmt = tx
      .prepare("SELECT COALESCE(MAX(position), 0) FROM collection_media WHERE collection_id = ?1")
      .map_err(|e| e.to_string())?;
    ...
    for media_id in media_ids {
      current_pos += 1;
      stmt
        .execute(rusqlite::params![id, media_id, current_pos])
        .map_err(|e| e.to_string())?;
    }
  }
...
#[tauri::command]
pub fn add_media_to_collection_batch(
  state: tauri::State<'_, DbState>,
  media_id: String,
...
pub fn create_collection(
  ...
  if data.collection_type != CollectionType::Manual {
    if let CollectionMediaType::Specific(media_type) = data.media_type {
      filter.media_type = Some(media_type);
    }
    filter_json =
      serde_json::to_string(&filter).map_err(|e| format!("JSON serialization error: {}", e))?;
  }
